...
def mjd_to_day_of_year(mjd_array):
    """Convert MJD to fractional day-of-year (0–365.25)."""
    # MJD 0 = 17 Nov 4713 BC; but we just need the fractional year position.
    # Use: day_of_year = (mjd mod 365.25)
    return mjd_array % ANNUAL_PERIOD
...
def extract_data(toas, fitter):
    ...
    return mjd, residuals_s, orb_phase, day_of_year
...
def bin_and_fit(mjd, residuals_us, orb_phase, day_of_year):
    """
    ...
    for i in range(N_PHASE_BINS):
        ...
        try:
            ...
            coeffs, _, _, _ = np.linalg.lstsq(M, r_bin.astype(np.float64), rcond=None)
...
def main():
    ...
    mjd, residuals_us, orb_phase, day_of_year = extract_data(toas, fitter)
    ...
    phase_centers_deg, results = bin_and_fit(mjd, residuals_us, orb_phase, day_of_year)
